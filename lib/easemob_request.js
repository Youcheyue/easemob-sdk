...
exports.get_token_object = function (callback) {
  var data = {grant_type: 'client_credentials', client_id: app_const.CLIENT_ID , client_secret: app_const.CLIENT_SECRET};
  request_json(data, '/token', 'POST', function (err,res,body) {
    if (!err && body){
      callback(null,body);
...
exports.request_accept = function ( file,headers,path,callback) {
  var options = {
    url     : app_const.BASE_URL + app_const.ORG_NAME + '/' + app_const.APP_NAME + path,
    headers : headers ,
  };
  ...
  request(options).pipe(fs.createWriteStream(file)).on('finish',callback);
  //request(options).on('close',callback).pipe(fs.createWriteStream(file));
};
